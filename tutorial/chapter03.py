...
async def lastest(city: CityName) -> dict:
...
@app03.get("/files/{filepath: path}")  # 通过路径参数传递文件的路径
def filepath(filepath: str) -> str:
    return "filepath is {}".format(filepath)


@app03.get("/path/{num}")  # 对传入路径数字参数的校验
def path_params_validate(num: int = Path(None, title="your number", description="不可描述", ge=1, le=10)) -> int:
    return num
...
# 加了optional 为选填参数
@app03.get("/query")
def page_limit(page: int = 1, limit: Optional[int] = None) -> dict:
    if limit:
        return {"page": page, limit: limit}
    return {"page": page}


@app03.get("/query/bool/conversion")  # bool类型转换: yes on 1 True true会转成为true
def type_conversion(param: bool = False) -> bool:
    return param


@app03.get("/query/validations")  # 多个查询参数的列表，参数别名
...
class CityInfo(BaseModel):
    name: str = Field(..., example="Beijing")  # example是注解的作用，不会被验证 ...表示必填参数
    country: str
...
def city_info(city: CityInfo) -> dict:
...
class Data(BaseModel):
    ...
    date: list # 额外的数据类型，还有uuid datetime bytes frozenset等，参考：https://fastapi.tiangolo.com/tutorial/extra-data-types/
